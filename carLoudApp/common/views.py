...
class IndexView(ListView):
    ...
    def get_queryset(self):
        images = ProjectImages.objects.filter(project__private=False).annotate(like_count=Count('likes')).order_by('-like_count')[:8]
        return images
...
class DashboardView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = ProjectImages.objects.filter(project__private=False)
